...
sql="select * from jwr_access_list  where access_id >666 limit 3;
	select * from jwr_access_list where access_id <10 limit 2 	
"
#-X 表示返回XML 格式数据
<< XML

<resultset statement="select * from jwr_access_list where access_id &lt;10 limit 2
...
#$mysql_cmd  -e "insert into jwr_access_list values(null,'182.168.1.101',100,'http://wd.juwairen.net',158374833,0,0,0)"
#-u 和用户，-p和密码 之间必须没有空格 ，否则会进行交互式提示输入密码 
# jwairen 指定连接的数据库名
$MYSQL -h 192.168.1.127 jwairen -usofar -psofar -e "insert into jwr_admin values(null,'test','passwd','love email','qq','15334324','192.168.1.102','4212121',1)"
